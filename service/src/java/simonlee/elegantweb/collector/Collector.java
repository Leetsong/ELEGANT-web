...
import simonlee.elegantweb.ELEGANTRunner;

import java.io.PrintStream;
import java.util.*;

// Reporter is the controller that controls the format of the output technique collect
public class Collector {
    ...
    private static final String REPORT_SUMMARY_TEMPLATE =
            ...
            "  ignore them if they are collected imprecisely\n";
    ...
    public Collector(ELEGANT elegant, boolean verbose) {
    ...
    /**
     * collect will collect all information saved in collected via ps
     *
     * @param ps a print stream
     */
    public void collect(PrintStream ps) {
        ps.printf(REPORT_HEAD_TEMPLATE,
                ELEGANTRunner.APP.NAME, 
                ELEGANTRunner.APP.VERSION, 
                ELEGANTRunner.APP.DESCRIPTION, 
                ELEGANTRunner.AUTHOR.NAME);
        ps.println();
        ...
        for (Map.Entry<ApiContext, Section> entry : sections.entrySet()) {
            ...
            collectApiContext(ps, acpair);
            collectAllInformation(ps, section);
            ps.println();
        }
        ...
        ps.printf(REPORT_FOOT_TEMPLATE, ELEGANTRunner.AUTHOR.EMAIL);
    }

    private void collectApiContext(PrintStream ps, ApiContext acpair) {
    ...
    private void collectAllInformation(PrintStream ps, Section section) {
